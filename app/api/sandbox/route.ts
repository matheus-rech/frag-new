...
export async function POST(req: Request) {
  ...
  if (fragment.template === 'shiny-r-developer' || fragment.template === 'shiny-python-developer') {
    const { logs, error } = await sbx.runCode(fragment.code || '')

    return new Response(
      JSON.stringify({
        sbxId: sbx?.sandboxId,
        template: fragment.template,
        url: `https://${sbx?.getHost(fragment.port || 8000)}`,
